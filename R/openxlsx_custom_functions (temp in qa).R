...
oxl_named_region_to_rowcol <- function(wb, sheet, named_region) {
  ...
  z %>%
    ...
    dplyr::mutate(fromcol = stringr::str_extract(from, "[A-Z]+"),
           tocol = stringr::str_extract(to, "[A-Z]+")) %>%
    dplyr::mutate(fromrow = stringr::str_extract(from, "[0-9]+"),
           torow = stringr::str_extract(to, "[0-9]+")) %>%
    dplyr::mutate(fromcol = letnum(fromcol),
           tocol = letnum(tocol))
}
