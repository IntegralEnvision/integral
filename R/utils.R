...
is_unsaved <- function(path, warn_only = F) {
  ...
  if(file_has_unsaved_changes) {
    cli::cli_alert_danger("{filepath} has unsaved changes.", wrap = T)
    return(invisible(TRUE))
  } else if(!file_has_unsaved_changes) {
    cli::cli_alert_success("{filepath}: No unsaved changes detected.", wrap = T)
