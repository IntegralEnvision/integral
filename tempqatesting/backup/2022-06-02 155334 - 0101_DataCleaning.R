...
# QA: files are brought in with logical
...
chem_dat2 <- chem_dat %>%
  ...
  # QA: change to snakecase
  clean_names() %>%
  # QA: rename to more recognizable names
  rename(sample_id = field_sample_id,
         analyte = parameter,
         concentration = numeric_result, 
         detected = is_detect) %>%
  # QA: drop the one sample that has a replicated field sample and analyte combo, keep the first analysis with higher concentration
  filter(!(sample_id == "WBMN-GW-S04SP-0-181205" & concentration == 0.0395)) %>%
  # QA: drop the samples with NA in concentration - no RDL reported
...
add_chem_dat2 <- add_chem_dat %>%
  # QA: rename colmns to match chem_dat2
  ...
  # QA: change units (they are equal, but make them the same)
  mutate(units = ifelse(units == "ug/L",
                        "ng/mL", units)) %>%
  # QA: change analyte names and join with longer names
  mutate(analyte_short  = analyte_short %>%
           str_replace(".ndDL", "")) %>%
  left_join(analyte_lookup_dat, by = "analyte_short") %>%
  # QA: convert sample_date into datetime object
...
vap_dat2 <- vap_dat %>%
  # QA: dump rows with no data
  remove_empty(which = c("rows"))%>%
  # QA: select columns with pertinant data 
  select(SampleLocation, SampleDate, `Identifier - table 6`, 
         contains('ug/L_ppb_result'),
         contains('Q'))%>%
  # QA: pivot concentrations
...
chem_dat2 <- chem_dat2 %>%
  group_by(sample_id) %>%
  # QA: drop samples where there are not all 5 analytes measured
  filter(n() == 5)
...
# QA: All columns are expected data type 
# QA: No NAs where we would not expect them
visdat::vis_dat(qa_check_df)


# SAVE DATA -----
# QA: Correct R object is saved and logically named
